    ...
    >
      <section className="flex flex-col lg:flex-row items-center justify-between w-full max-xl:gap-6 max-w-7xl lg:max-w-6xl">
        <p className="max-md:font-medium text-3xl md:text-5xl lg:text-6xl xl:text-7xl lg:max-w-lg xl:max-w-2xl tracking-tighter text-center lg:text-left">
          MylesCRM for teams that sell, follow up, and collect with discipline
        </p>
        <section className="flex flex-col gap-8">
          <p className="text-md md:text-xl max-w-xl lg:max-w-md text-center lg:text-left">
            Manage contacts, deals, invoices, follow-ups, and AI-assisted sales
            work in one polished MylesCorp workspace built for East African teams.
          </p>
          <div className="flex flex-row gap-2">
            <Button render={<Link href="/contact" />} className="rounded-full max-lg:hidden">
              Book a call
            </Button>
            <Button render={<Link href="/features" />} className="rounded-full max-lg:w-full" variant="outline">
              Explore features
            </Button>
          </div>
        </section>
