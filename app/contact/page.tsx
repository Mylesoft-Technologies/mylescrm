            ...
            <div className="grid gap-6 lg:grid-cols-[0.9fr_1.1fr]">
              <div className="space-y-4">
                {contactMethods.map((method) => (
                  <a key={method.label} href={method.href} className="flex items-start gap-4 rounded-lg border border-border bg-card p-5 shadow-sm hover:border-primary">
                    <method.icon className="mt-1 h-5 w-5 text-primary" />
                    <span>
                      <span className="block font-semibold text-foreground">{method.label}</span>
                      <span className="mt-1 block text-sm text-muted-foreground">{method.value}</span>
                    </span>
                  </a>
                ))}
              </div>
              <div className="rounded-lg border border-border bg-card p-6 shadow-sm">
                <h2 className="text-xl font-semibold tracking-tight text-foreground">Request a demo</h2>
                ...
                <div className="mt-6 grid gap-4 md:grid-cols-2">
                  {["Company size", "Sales team size", "Current CRM", "Primary goal"].map((label) => (
                    <div key={label} className="rounded-lg bg-muted/40 p-4">
                      ...
                      <p className="mt-2 text-sm text-muted-foreground">Include this in your message</p>
                    </div>
                  ))}
                </div>
                <a
                  href="mailto:support@mylescorptech.com?subject=MylesCRM%20Demo%20Request&body=Hi%20MylesCRM%20team%2C%0A%0AI%20would%20like%20a%20professional%20demo.%0A%0ACompany%3A%0ASales%20team%20size%3A%0ACurrent%20CRM%3A%0APrimary%20goal%3A%0A%0AThank%20you."
          ...
          <div className="mx-auto max-w-6xl">
            ...
            <div className="grid gap-4 md:grid-cols-4">
              {[
                ...
                <div key={title} className="rounded-lg border border-border bg-muted/40 p-5">
                  <span className="flex h-9 w-9 items-center justify-center rounded-lg bg-primary text-sm font-semibold text-primary-foreground">{step}</span>
